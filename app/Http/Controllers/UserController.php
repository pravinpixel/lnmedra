...
class UserController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('users-add')) {
            $notIncludeRole = config('global.vendor');
            $lims_role_list = Roles::whereNotIn('name', $notIncludeRole)->where('is_active', true)->get();
    ...
    {
        $lims_user_data = User::find($id)->toArray();
        if ($lims_user_data['vendor_id']) {
            $lims_supplier_data = Supplier::where('id', $lims_user_data['vendor_id'])->select('nursery_code')->first()->toArray();
            $lims_user_data =  array_merge($lims_user_data, $lims_supplier_data);
        }
    ...
    public function profileUpdate(Request $request, $id)
    {
        ...
        if (isset($input['nursery_code'])) {
            $lims_user_data = Supplier::where('id', $lims_user_data->vendor_id)->first();
